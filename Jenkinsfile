      ...
      stage('Pushing Image'){
        ...
        steps{
          script{ 
                   withMaven(globalMavenSettingsConfig: '', jdk: '', maven: 'Maven', mavenSettingsConfig: '', traceability: true) {
                   sh  'mvn clean deploy'
                    sh 'docker tag login-test:$BUILD_NUMBER http://172.29.158.73:8081/repository/docker-images/login-test:$BUILD_NUMBER'
                    sh 'docker images'
                    // withCredentials([usernamePassword(credentialsId: 'docker', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                    //    sh 'docker login -u $DOCKER_USERNAME -p $DOCKER_PASSWORD'
                     docker login -u admin -p admin123 http://172.29.158.73:8081
                       sh 'docker push http://172.29.158.73:8081/repository/docker-images/login-test:$BUILD_NUMBER'
          }
        }
      }
   }
